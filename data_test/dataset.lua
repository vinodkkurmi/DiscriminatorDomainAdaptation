...
-- Modified by Vinod Kumar Kurmi(vinodkumarkurmi@gmail.com)
...
-- getByClass
function dataset:getByClass(index)
   local imgpath = ffi.string(torch.data(self.imagePath[index]))
   -- local imgpath = ffi.string(torch.data(self.imagePath[self.classListSample[class][index]]))
  -- print('imgpath new',imgpath ,self.imageClass[index])

   return self:sampleHookTest(imgpath)
end
...
function dataset:sampleSequence(quantity,start_Batch_IndexTest)
   assert(quantity)
   local dataTable = {}
   local scalarTable = {}
   local startindex=start_Batch_IndexTest
   local endindex=start_Batch_IndexTest+quantity-1
   -- print('opt.batchIndex',opt.batchIndex)
   -- print('startindex',startindex)
   -- print('endindex',endindex)
   for i=startindex,endindex do
      local class = torch.random(1, #self.classes)
      local out = self:getByClass(i)
      table.insert(dataTable, out)
      table.insert(scalarTable, self.imageClass[i])
   end
